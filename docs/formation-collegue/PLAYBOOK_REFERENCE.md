...
| Export UGC TikTok 9:16 | `before-after-export.ts`, `BeforeAfterShareButton` (post-génération) |
...
| Post-paiement fluide | Stripe → `/resultat?checkout=success`, pas auto-gen + ✨ |
| Bouton ✨ transformation TikTok | Sur `UnlockedLarpView`, pas sur `/resultat` principal |

### ❌ Pas encore fait

| Recommandation | Priorité suggérée |
|----------------|-------------------|
| Preuve sociale paywall | Moyenne |
...
```
Landing → Register → Quiz (2 étapes) → Fake loader → /image-prete → Paywall → Paiement → /resultat
```

**Parcours cible playbook** :
...
| Stripe success | `server/stripe-routes.ts` → `/resultat?checkout=success` |
