...
export default function VideoPrete() {
  ...
  const {
    ...
    isUrgent,
    purgeExpiredPreview,
  } = usePaywallPreviewPage("video", profile?.id);
  ...
  useEffect(() => {
    ...
    if (params.get("paywall") === "1" && !expired && (imageUrl || userPrompt)) {
      setPaywallOpen(true);
    }
  }, [location, expired, imageUrl, userPrompt, setPaywallOpen]);
      ...
      <div className="pointer-events-none fixed inset-x-0 top-0 z-[120] flex items-center justify-end gap-2 px-3 pt-[max(0.75rem,env(safe-area-inset-top))] sm:px-5">
        {!expired ? (
          <button
            ...
            className="pointer-events-auto flex max-w-[46%] shrink-0 items-center gap-1 rounded-lg border border-[var(--lx-gold)]/35 bg-[var(--lx-surface-2)]/95 px-2.5 py-1.5 text-sm font-semibold shadow-sm backdrop-blur-xl sm:max-w-none sm:gap-1.5 sm:px-3"
          >
        ...
        <button
          ...
          className="pointer-events-auto inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-[var(--lx-gold)]/35 bg-[var(--lx-surface-2)]/95 px-2.5 py-1.5 text-sm font-semibold shadow-sm backdrop-blur-xl sm:px-3"
        >
            ...
            <header className="w-full text-center">
              ...
              <p className="mx-auto mt-2 max-w-sm text-sm text-[var(--lx-muted)]">
                {userPrompt
                  ...
                  : "Débloque pour télécharger ta vidéo IA."}
              </p>
            </header>
