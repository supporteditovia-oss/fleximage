...
import type { VoiceSharePlatform } from "@/lib/share-voice";

const PLATFORMS: {
  id: VoiceSharePlatform;
  name: string;
  hint: string;
  icon: JSX.Element;
}[] = [
...
type VoiceShareSheetProps = {
  open: boolean;
  onClose: () => void;
  onSelect: (platform: VoiceSharePlatform) => void;
};

export function VoiceShareSheet({ open, onClose, onSelect }: VoiceShareSheetProps) {
  if (!open) return null;
    ...
    <div className="pointer-events-none fixed inset-0 z-[200] flex items-end justify-center">
      ...
      >
        ...
        <div className="relative mb-1 text-center">
          ...
          <p className="text-base font-semibold text-foreground">Partager le vocal</p>
          <p className="mt-1 text-xs text-muted-foreground">
            Android : WhatsApp s&apos;ouvre direct avec le vocal. Ensuite choisis ton contact.
          </p>
        </div>
        <div className="grid grid-cols-3 gap-2 px-0 pb-2 pt-3">
          {PLATFORMS.map((platform) => (
            ...
            >
              <div className="flex h-14 w-14 items-center justify-center rounded-full border border-border/30 bg-card text-primary shadow-md">
                {platform.icon}
              </div>
              <span className="text-xs font-semibold">{platform.name}</span>
              <span className="text-[10px] text-muted-foreground">{platform.hint}</span>
            </button>
          ))}
        </div>
      </div>
