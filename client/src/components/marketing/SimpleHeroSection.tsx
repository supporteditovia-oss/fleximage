...
export default function SimpleHeroSection() {
  ...
  const handleStart = () => {
    ...
    posthog?.capture("landing_simple_cta_clicked", {
      landing_variant: LANDING_EXPERIMENT_SIMPLE_VARIANT,
      destination,
      [`$feature/${LANDING_EXPERIMENT_FLAG_KEY}`]:
        LANDING_EXPERIMENT_SIMPLE_VARIANT,
    });
    ...
    <section className="relative flex min-h-[68svh] flex-col items-center justify-start overflow-hidden px-4 pb-4 pt-28 min-[380px]:pt-32 md:min-h-[70svh] md:justify-center md:pb-6 md:pt-20">
      ...
      >
        <motion.div variants={itemVariants} className="mb-5 md:mb-6">
          <div className="inline-flex items-center gap-2 rounded-full border border-border/60 bg-muted/40 px-3.5 py-1.5 text-xs font-semibold text-muted-foreground shadow-sm backdrop-blur-md md:px-4 md:py-2 md:text-sm">
        ...
        <motion.div variants={itemVariants} className="mt-8 md:mt-12">
          <Button
            onClick={handleStart}
            className="group h-12 rounded-full border-0 bg-gradient-to-r from-primary to-secondary px-7 text-sm font-bold text-primary-foreground shadow-[0_14px_44px_hsl(var(--primary)/0.2)] transition-transform active:scale-95 min-[380px]:px-8 min-[380px]:text-base md:h-[3.75rem] md:px-11 md:text-[1.05rem]"
          >
            {t("hero.simpleCta")}
