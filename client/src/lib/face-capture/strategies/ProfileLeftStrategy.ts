...
export class ProfileLeftStrategy implements PoseStrategy {
  ...
  evaluate(frame: FaceFrame, pose: PoseDefinition, opts?: StrategyOptions) {
    ...
    if (!inRange(y, yawRange)) {
      if (y < yLo) hints.push("Tournez un peu vers la droite.");
      else hints.push("Tournez davantage la tête vers la gauche.");
    }
    ...
    /** Barre d’alignement / score : uniquement le yaw (angle du profil). */
